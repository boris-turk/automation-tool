	...
	public class WlanClient
	{
		...
		public class WlanInterface
		{
			...
			public Wlan.WlanConnectionAttributes CurrentConnection
			{
				get
				{
					...
					Wlan.WlanOpcodeValueType opcodeValueType;
					Wlan.ThrowIfError(
						Wlan.WlanQueryInterface(_client._clientHandle, _info.interfaceGuid, Wlan.WlanIntfOpcode.CurrentConnection, IntPtr.Zero, out valueSize, out valuePtr, out opcodeValueType));
					try
					{
			...
			{
				Wlan.ThrowIfError(
					Wlan.WlanScan(_client._clientHandle, _info.interfaceGuid, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero));
			}
			...
			public Wlan.WlanBssEntry[] GetNetworkBssList()
			{
				IntPtr bssListPtr;
				Wlan.ThrowIfError(
					Wlan.WlanGetNetworkBssList(_client._clientHandle, _info.interfaceGuid, IntPtr.Zero, Wlan.Dot11BssType.Any, false, IntPtr.Zero, out bssListPtr));
				try
				{
			...
			{
				Wlan.ThrowIfError(
					Wlan.WlanDeleteProfile(_client._clientHandle, _info.interfaceGuid, profileName, IntPtr.Zero));
			}
			...
			public string GetProfileXml(string profileName)
			{
				...
				Wlan.ThrowIfError(
					Wlan.WlanGetProfile(_client._clientHandle, _info.interfaceGuid, profileName, IntPtr.Zero, out profileXmlPtr, out flags,
					               out access));
				try
				{
			...
			public Wlan.WlanProfileInfo[] GetProfiles()
			{
				...
				Wlan.ThrowIfError(
					Wlan.WlanGetProfileList(_client._clientHandle, _info.interfaceGuid, IntPtr.Zero, out profileListPtr));
				try
				{
		...
		public WlanInterface[] Interfaces
		{
			get
			{
				IntPtr ifaceList;
				Wlan.ThrowIfError(
					Wlan.WlanEnumInterfaces(_clientHandle, IntPtr.Zero, out ifaceList));
				try
				{
